...
# Tự động tìm và nạp biến môi trường từ file .env
...
class ChatbotBaseline:
    ...
    def query(self, user_input: str) -> Dict[str, Any]:
        """
        Gửi yêu cầu tới mô hình ngôn ngữ mà không dùng công cụ tra cứu hay ReAct loop.
        Dễ gặp hiện tượng ảo giác (hallucination) hoặc từ chối do không có kết nối cơ sở dữ liệu.
        """
        if self.api_key:
            try:
                ...
                interaction = client.interactions.create(
                    ...
                    input=f"Bạn là chatbot tư vấn du lịch. Hãy trả lời câu hỏi sau của khách hàng mà KHÔNG dùng tool hay internet: {user_input}",
                )
        ...
        # Phản hồi dự phòng chuẩn nếu không có API key hoặc mất kết nối mạng
        return {
...
class ReActAgent:
    ...
    def parse_city_code(self, text: str) -> str:
        """Chuẩn hoá và trích xuất mã sân bay thành phố từ câu truy vấn"""
    ...
    def plan_and_execute_step(self, user_input: str, iteration: int) -> Tuple[str, bool]:
        ...
        Hỗ trợ truy vấn nhiều bước (multi-step), đơn bước (single-step), FAQ và safeguard.
        """
        user_lower = user_input.lower()
        
        # 1. Kiểm tra câu hỏi FAQ chung (không cần gọi tool)
        ...
        if needs_flight and iteration == 1:
            ...
            # Trap 1: Chuẩn hoá tên tool strip() và lower()
            ...
            # Nếu truy vấn chỉ hỏi chuyến bay, kết luận ngay tại bước 1
        ...
        # 3. Xử lý bước tra cứu thời tiết
            ...
            # Nếu truy vấn chỉ hỏi thời tiết, kết luận ngay tại bước 1
            if not needs_flight:
        ...
        # 4. Tổng hợp Final Answer khi đã thu thập đủ thông tin
        else:
    ...
    def run(self, user_input: str) -> Dict[str, Any]:
        """
        Chạy vòng lặp ReAct cho câu hỏi người dùng:
        ...
        # Safeguard khi chạm giới hạn số bước tối đa
        return {
...
def main():
    ...
    baseline_res = chatbot.query(user_query)
    print("Baseline Answer:", baseline_res.get("answer"))
    print("Baseline Status:", baseline_res.get("status"))
    
    print("\n=== RUNNING REACT AGENT ===")
    agent = ReActAgent(max_iterations=5)
    result = agent.run(user_query)
    print("Result Status:", result.get("status"))
    print("Total Iterations:", result.get("iterations"))
    print("Final Answer:\n", result.get("answer"))
